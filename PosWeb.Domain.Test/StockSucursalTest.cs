using PosWeb.Domain.Exceptions;
using Xunit;

namespace PosWeb.Domain.Test;

public class StockSucursalTest
{
    [Fact]
    public void Constructor_AsignaStockInicial()
    {
        var stock = new StockSucursal(1, 2, 10);

        Assert.Equal(1, stock.IdProducto);
        Assert.Equal(2, stock.IdSucursal);
        Assert.Equal(10, stock.Stock);
    }

    [Fact]
    public void Constructor_StockNegativo_LanzaExcepcion()
    {
        var ex = Assert.Throws<ArgumentException>(() => new StockSucursal(1, 2, -1));
        Assert.Contains("negativo", ex.Message);
    }

    [Fact]
    public void Constructor_StockCero_EsValido()
    {
        var stock = new StockSucursal(1, 2, 0);
        Assert.Equal(0, stock.Stock);
    }

    [Fact]
    public void DescontarStock_RestaCantidadCorrectamente()
    {
        var stock = new StockSucursal(1, 2, 10);

        stock.DescontarStock(3);

        Assert.Equal(7, stock.Stock);
    }

    [Fact]
    public void DescontarStock_StockInsuficiente_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 2);

        var ex = Assert.Throws<StockInsuficienteException>(() => stock.DescontarStock(5));

        Assert.Equal(2, ex.StockDisponible);
        Assert.Equal(5, ex.CantidadSolicitada);
        Assert.Equal(2, ex.IdSucursal);
    }

    [Fact]
    public void DescontarStock_CantidadCero_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 10);

        Assert.Throws<CantidadInvalidaException>(() => stock.DescontarStock(0));
    }

    [Fact]
    public void DescontarStock_CantidadNegativa_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 10);

        Assert.Throws<CantidadInvalidaException>(() => stock.DescontarStock(-1));
    }

    [Fact]
    public void DescontarStock_StockExacto_Permite()
    {
        var stock = new StockSucursal(1, 2, 5);

        stock.DescontarStock(5);

        Assert.Equal(0, stock.Stock);
    }

    [Fact]
    public void AumentarStock_SumaCantidadCorrectamente()
    {
        var stock = new StockSucursal(1, 2, 5);

        stock.AumentarStock(10);

        Assert.Equal(15, stock.Stock);
    }

    [Fact]
    public void AumentarStock_CantidadCero_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 5);

        Assert.Throws<CantidadInvalidaException>(() => stock.AumentarStock(0));
    }

    [Fact]
    public void AumentarStock_CantidadNegativa_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 5);

        Assert.Throws<CantidadInvalidaException>(() => stock.AumentarStock(-1));
    }

    [Fact]
    public void AumentarStock_DesdeCero_SumaCorrectamente()
    {
        var stock = new StockSucursal(1, 2, 0);

        stock.AumentarStock(1);

        Assert.Equal(1, stock.Stock);
    }

    [Fact]
    public void AjustarStock_CambiaStock()
    {
        var stock = new StockSucursal(1, 2, 5);

        stock.AjustarStock(20);

        Assert.Equal(20, stock.Stock);
    }

    [Fact]
    public void AjustarStock_ACero_EsValido()
    {
        var stock = new StockSucursal(1, 2, 5);

        stock.AjustarStock(0);

        Assert.Equal(0, stock.Stock);
    }

    [Fact]
    public void AjustarStock_Negativo_LanzaExcepcion()
    {
        var stock = new StockSucursal(1, 2, 5);

        Assert.Throws<StockInvalidoException>(() => stock.AjustarStock(-1));
    }
}
