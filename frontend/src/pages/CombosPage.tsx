...
export default function CombosPage() {
  ...
  const [confirmProps, setConfirmProps] = useState<{ open: boolean; title: string; message: string; onConfirm: () => void }>({ open: false, title: '', message: '', onConfirm: () => {} })
  ...
  useEffect(() => {
    Promise.all([
      api.productos.listar().then(setProductos),
      api.combos.listar().then(setCombos),
      api.ofertas.listar().then(setOfertas),
  ...
  async function handleEliminarDefinitivoCombo(id: number) {
    setConfirmProps({
      ...
      message: '¿Eliminar este combo permanentemente? Esta acción no se puede deshacer.',
      onConfirm: async () => {
  ...
  async function handleEliminarDefinitivoOferta(id: number) {
    setConfirmProps({
      ...
      message: '¿Eliminar esta oferta permanentemente? Esta acción no se puede deshacer.',
      onConfirm: async () => {
        setConfirmProps(p => ({ ...p, open: false }))
        setSaving(true)
        try {
  ...
  return (
    <PageShell
      ...
      subtitle={tab === 'combos' ? `${combos.length} combos` : `${ofertas.length} ofertas`}
      actions={
      ...
      loading={dataLoading}
      loadingMessage="Cargando combos y ofertas..."
    >
      <div className="space-y-5">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCombos.map(combo => (
                <Card key={combo.id} padding="md" className={`transition-all ${combo.activo ? 'hover:border-indigo-300 hover:shadow-md' : 'opacity-50'}`}>
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredOfertas.map(oferta => {
                ...
                return (
                  <Card key={oferta.id} padding="md" className={`transition-all ${!oferta.activo ? 'opacity-50' : vigente ? 'border-amber-200 hover:border-amber-400 hover:shadow-md' : 'opacity-60'}`}>
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
        ...
        <Dialog
          ...
          title={comboEditId !== null ? 'Editar combo' : 'Nuevo combo'}
          width="lg"
          footer={
            <button type="submit" form="combo-form" disabled={saving}
              ...
              {saving ? 'Guardando...' : comboEditId !== null ? 'Guardar cambios' : 'Crear combo'}
            </button>
          }
        >
          <ComboFormModal
        ...
        <Dialog
          ...
          title={ofertaEditId !== null ? 'Editar oferta' : 'Nueva oferta'}
          width="lg"
          footer={
            <button type="submit" form="oferta-form" disabled={saving}
              className="w-full py-3 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
              {saving ? 'Guardando...' : ofertaEditId !== null ? 'Guardar cambios' : 'Crear oferta'}
            </button>
          }
        >
          <OfertaFormModal
        ...
        <Dialog
          ...
          description={confirmProps.message}
          width="sm"
          footer={
            <div className="flex gap-3 w-full justify-end">
              <Button variant="secondary" size="md" onClick={() => setConfirmProps(p => ({ ...p, open: false }))}>
                Cancelar
              </Button>
              <Button variant="destructive" size="md" onClick={confirmProps.onConfirm}>
                Eliminar
              </Button>
            </div>
          }
...
function ComboFormModal({
  form, setForm, editId, productos, onSubmit, notifyError,
}: {
  ...
  return (
    <form id="combo-form" onSubmit={onSubmit} className="space-y-4">
...
function OfertaFormModal({
  form, setForm, editId, productos, onSubmit,
}: {
  ...
  return (
    <form id="oferta-form" onSubmit={onSubmit} className="space-y-4">
